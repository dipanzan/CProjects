...
#define CPUINFO_TRANSITION_LATENCY 131072 // transition_latency is in nano-seconds (ns) !!

// sys/devices/system/cpu/cpufreq/ondemand > sampling_rate
#define DEFAULT_SAMPLING_RATE 8000	  // sampling_rate is in micro-seconds (us) !!
#define INCREASED_SAMPLING_RATE 10000 // us

#define TARGET_PROCESS_NAME "rapl"

static struct task_struct *target_process;
...
static void hook_process(void)
{
	pr_info("Hooking with target process: [%s]\n", TARGET_PROCESS_NAME);
	...
	for_each_process(_process)
	{
		...
		{
			target_process = _process;
			pr_alert("HOOKED with target process: [%s]\n", _process->comm);
			break;
		}
	}
	rcu_read_unlock();
...
static void dbs_freq_default(struct cpufreq_policy *policy, int cpu_load)
{
	...
	// pr_alert("%s: cpu: %u freq_next: %u KHz", __func__, policy->cpu, freq_next);
	__cpufreq_driver_target(policy, freq_next, CPUFREQ_RELATION_C);
}

static void dbs_freq_hook(struct cpufreq_policy *policy, int cpu_load)
{
	struct policy_dbs_info *policy_dbs = policy->governor_data;
	struct od_policy_dbs_info *dbs_info = to_dbs_info(policy_dbs);
	struct dbs_data *dbs_data = policy_dbs->dbs_data;
	...
	if (target_process != NULL) {
		rcu_read_lock();
		for_each_thread(target_process, _thread)
		{
			if (policy->cpu == _thread->cpu)
			{
				target_cpu = true;
				break;
			}
	...
	target_cpu ? __cpufreq_driver_target(policy, CPU_MAX_FREQUENCY, CPUFREQ_RELATION_C)
			   : __cpufreq_driver_target(policy, CPU_MIN_FREQUENCY, CPUFREQ_RELATION_C);
}
...
static void od_update(struct cpufreq_policy *policy)
{
	...
	{
		// abusing the branch predictor hehe
		if (likely(target_process != NULL))
		{
			dbs_freq_hook(policy, cpu_load);
		}
		else
		{
			dbs_freq_default(policy, cpu_load);
		}
	}
}
...
static unsigned int od_dbs_update(struct cpufreq_policy *policy)
{
	...
	if (unlikely(target_process == NULL))
	{
		hook_process();
	}
	od_update(policy);
...
static void od_start(struct cpufreq_policy *policy)
{
	...
	if (dbs_info->policy_dbs.dbs_data != NULL)
	{
		pr_alert("sampling rate: %d\n", dbs_info->policy_dbs.dbs_data->sampling_rate);
	}

	dbs_info->policy_dbs.dbs_data->sampling_rate = 10000;
	dbs_info->sample_type = OD_NORMAL_SAMPLE;
